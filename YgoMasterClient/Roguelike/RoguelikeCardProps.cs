...
{
    // Static card properties by cid (race/attr/level/subType/atk/def), resolved from the game's CARD_Prop
    // data via YdkHelper.GameCardInfo (PropA/PropB decode) and cached. Single source for both the
    // RoguelikeStatBuff subType filter and the duelHook get_card_props, so buff rules and Lua hooks agree.
    ...
    static class RoguelikeCardProps
    {
        public struct Props
        {
            ...
            public string SubType;   // normal/effect/fusion/synchro/xyz/link/ritual/pendulum/spell/trap/token/god
        ...
        // subType/peek need so the duel hot path doesn't pay unless a rule/script actually asks.
        ...
        public static bool TryGet(int cid, out Props p)
        {
            ...
            p.Atk = gi.Atk;
            p.Def = gi.Def;
            p.SubType = FrameToSubType(gi.Frame);
        ...
        // CardFrame -> the lowercase subType token used by buff rules and Lua filters (archetype-agnostic).
        public static string FrameToSubType(CardFrame f)
        {
            switch (f)
            {
